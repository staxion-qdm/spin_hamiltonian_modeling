...
class StevensOperatorsMS:
    def __init__(self, so, label):        
        # Define constants
        Sval = so.get_Sval(label)
        Sp = so.get_op(label, 'Sp')
        Sm = so.get_op(label, 'Sm')
        Sz = so.get_op(label, 'Sz')
        s = Sval * (Sval + 1)
        cp = 1/2
        cm = 1/(2j)
        II = np.eye(so.dim)
        ...
        # k = 2
        self.O_2 = {
            -2: cm * (sops.SO_power(Sp, 2) - cm * sops.SO_power(Sm, 2)),
            -1: cm * sops.symmprod(Sz, Sp - Sm),
            0: 3 * sops.SO_power(Sz, 2) - s * II,
            1: cp * sops.symmprod(Sz, Sp + Sm),
            2: cp * (sops.SO_power(Sp, 2) + sops.SO_power(Sm, 2))
        }

        # k = 4
        self.O_4 = {
            -4: cm * (sops.SO_power(Sp, 4) - sops.SO_power(Sm, 4)),
            -3: cm * sops.symmprod(Sz, sops.SO_power(Sp, 3) - sops.SO_power(Sm, 3)),
            -2: cm * sops.symmprod(7 * sops.SO_power(Sz, 2) - (s + 5)*II, sops.SO_power(Sp, 2) - sops.SO_power(Sm, 2)),
            -1: cm * sops.symmprod(7 * sops.SO_power(Sz, 3) - (3*s + 1)*Sz, Sp - Sm),
            0: 35 * sops.SO_power(Sz, 4) - (30*s - 25) * sops.SO_power(Sz, 2) + (3*s**2 - 6*s) * II,
            1: cp * sops.symmprod(7 * sops.SO_power(Sz, 3) - (3*s + 1)*Sz, Sp + Sm),
            2: cp * sops.symmprod(7 * sops.SO_power(Sz, 2) - (s + 5)*II, sops.SO_power(Sp, 2) + sops.SO_power(Sm, 2)),
            3: cp * sops.symmprod(Sz, sops.SO_power(Sp, 3) + sops.SO_power(Sm, 3)),
            4: cp * (sops.SO_power(Sp, 4) + sops.SO_power(Sm, 4))
        }

        # k = 6
        self.O_6 = {
            -6: cm * (sops.SO_power(Sp, 6) - sops.SO_power(Sm, 6)),
            -5: cm * sops.symmprod(Sz, sops.SO_power(Sp, 5) - sops.SO_power(Sm, 5)),
            -4: cm * sops.symmprod(
                11 * sops.SO_power(Sz, 2) - (s + 38) * II, 
                sops.SO_power(Sp, 4) - sops.SO_power(Sm, 4)
                ),
            -3: cm * sops.symmprod(
                11 * sops.SO_power(Sz, 3) - (3*s + 59) * Sz,
                sops.SO_power(Sp, 3) - sops.SO_power(Sm, 3)
                ),
            -2: cm * sops.symmprod(
                33 * sops.SO_power(Sz, 4)
                - (18*s + 123) * sops.SO_power(Sz, 2)
                + (s**2 + 10*s + 102) * II,
                sops.SO_power(Sp, 2) - sops.SO_power(Sm, 2)
                ),
            -1: cm * sops.symmprod(
                33 * sops.SO_power(Sz, 5)
                - (30*s - 15) * sops.SO_power(Sz, 3)
                + (5*s**2 - 10*s + 12) * Sz,
                Sp - Sm
                ),
            0: 231 * sops.SO_power(Sz, 6)
                - (315*s - 735) * sops.SO_power(Sz, 4)
                + (105*s**2 - 525*s + 294) * sops.SO_power(Sz, 2)
                - (5*s**3 - 40*s**2 + 60*s) * II,
            1: cp * sops.symmprod(
                33 * sops.SO_power(Sz, 5)
                - (30*s - 15) * sops.SO_power(Sz, 3)
            ...
            5: cp * sops.symmprod(Sz, sops.SO_power(Sp, 5) + sops.SO_power(Sm, 5)),
            6: cp * (sops.SO_power(Sp, 6) + sops.SO_power(Sm, 6))
        }